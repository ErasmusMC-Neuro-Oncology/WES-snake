...
option_list <- list(
    ...
    make_option(c("--margin"), type = "double", default = 0.05,
...
#opt$variants <- "/home/jurriaan/mnt/BIGR_home/SSLOWGRADE/output/WES/PureCN/500kbp/SG_021/SG_021_tumor1_variants.csv"
#opt$ACE_results <- "/home/jurriaan/mnt/BIGR_home/SSLOWGRADE/output/WES/ACE/500kbp/SG_021/ACE_fits.txt"
#opt$seg_file <- "/home/jurriaan/mnt/BIGR_home/SSLOWGRADE/output/WES/QDNAseq/500kbp/SG_021/data/QDNAseq_Segments.txt"
#-------------------------------------------------------------------------------
# 1.1  Read files
#-------------------------------------------------------------------------------
...
if (nrow(TP53_variants) != 0) {
    ...
    if (nrow(TP53_scored) > 1) {
        flog.warn("Multiple TP53 somatic calls found (%d); using highest-depth call.", nrow(TP53_scored))
        TP53_scored <- TP53_scored %>% slice_max(depth, n = 1, with_ties = FALSE)
    }
...
select_purity_from_ACE <- function(ACE_results, anchor_purities,
    ...
    candidates <- ACE_results %>% filter(minimum == TRUE)
...
#-------------------------------------------------------------------------------
final_purity <- ace_selection$chosen$cellularity
final_ploidy <- ace_selection$chosen$ploidy
...
build_purity_ploidy_bounds <- function(ace_selection, n_anchors,
                                        margin = 0.05,
                                        ploidy_margin = 0.3,
    ...
    chosen <- ace_selection$chosen
    final_purity <- chosen$cellularity
    final_ploidy <- chosen$ploidy
    ...
    if ((low_confidence || extreme_ploidy) &&
        abs(final_ploidy - low_confidence_ploidy_target) > ploidy_margin) {

        flog.warn(
            "Chosen ploidy (%.2f) is %s; widening search to include %.1f (%s).",
            final_ploidy,
            if (extreme_ploidy) "near-haploid/high-ploidy (likely doubling artifact)" else "far from 2N with few anchors",
            low_confidence_ploidy_target,
            if (low_confidence) sprintf("%d anchor(s)", n_anchors) else "extreme ploidy trigger"
        )
...
bounds <- build_purity_ploidy_bounds(ace_selection, n_anchors = sum(!is.na(anchor_values)))
...
result <- data.frame(
    ...
    ace_ploidy = final_ploidy,
    ace_purity = final_purity,
